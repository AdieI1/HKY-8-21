  ...
  const dispatchDelivery = async () => {
    ...
    try {
      await api.post(`/deliveries/${selectedDelivery.delivery_id}/dispatch`, {
        driver_id: chosenDriverId, vehicle_id: chosenVehicleId,
        trip_date: tripDate, fuel_issued: fuelLiters || null,
        fuel_receipt_no: fuelReceiptNo || null, remarks: remarks.trim() || null,
      });
    } catch (err) {
