    ...
    <>
      ...
      {showDetailsModal && detailsDriver && (
        <div className="modal" id="driverDetailsModal" style={{ display: 'block' }}>
          <div className="modal-content" style={{ maxWidth: 650, borderRadius: 12, overflow: 'hidden' }}>
            <div className="modal-header" style={{ background: '#d32f2f', color: '#fff', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ color: '#fff', margin: 0, fontSize: '18px' }}>DRIVER DETAILS</h2>
              <button className="btn-cancel" onClick={() => setShowDetailsModal(false)} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: '#fff', borderRadius: '50%', width: '32px', height: '32px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <i className="fas fa-times"></i>
              </button>
            </div>
            <div className="modal-body" style={{ padding: 24 }}>
              {/* Driver hero banner */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingBottom: 16, borderBottom: '1px solid #e2e8f0', marginBottom: 16 }}>
                <img
                  src={detailsDriver.user?.profile_photo_url || '/images/brucednegrow.png'}
                  alt={detailsDriver.user?.full_name}
                  style={{ width: 80, height: 80, borderRadius: '50%', objectFit: 'cover', border: '3px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}
                  onError={(e) => { e.currentTarget.src = '/images/brucednegrow.png'; }}
                />
                <div>
                  <h3 style={{ margin: '0 0 4px', fontSize: 20 }}>{detailsDriver.user?.full_name}</h3>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 13, color: '#64748b', fontWeight: 600 }}>{driverCode(detailsDriver.driver_id)}</span>
                    <span className={`driver-status ${availabilityClass(detailsDriver)}`} style={{ padding: '2px 8px', borderRadius: 4, fontSize: 12 }}>
                      {availabilityLabel(detailsDriver)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  <p style={{ margin: '6px 0' }}><strong>Phone:</strong> {detailsDriver.user?.phone || '—'}</p>
                  <p style={{ margin: '6px 0' }}><strong>Email:</strong> {detailsDriver.user?.email || '—'}</p>
                  <p style={{ margin: '6px 0' }}><strong>Birthdate:</strong> {formatDate(detailsDriver.birthdate)}</p>
                  <p style={{ margin: '6px 0' }}><strong>Nationality:</strong> {detailsDriver.nationality || '—'}</p>
                </div>
                <div>
                  <p style={{ margin: '6px 0' }}><strong>License No:</strong> {detailsDriver.license_number || '—'}</p>
                  <p style={{ margin: '6px 0' }}><strong>License Type:</strong> {detailsDriver.license_type || '—'}</p>
                  <p style={{ margin: '6px 0' }}><strong>Restriction Code:</strong> {detailsDriver.restriction_code || 'None'}</p>
                  <p style={{ margin: '6px 0' }}><strong>License Expiry:</strong> {formatDate(detailsDriver.license_expiry_date)}</p>
                </div>
              </div>

              <h4 style={{ margin: '16px 0 8px', borderBottom: '1px solid #f1f5f9', paddingBottom: 4 }}>Health & Employment</h4>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  <p style={{ margin: '6px 0' }}><strong>Health Condition:</strong> {detailsDriver.health_condition || '—'}</p>
                  <p style={{ margin: '6px 0' }}><strong>Last Medical:</strong> {formatDate(detailsDriver.last_medical_check)}</p>
                  <p style={{ margin: '6px 0' }}><strong>Prescriptions:</strong> {detailsDriver.prescriptions || 'None'}</p>
                </div>
                <div>
                  <p style={{ margin: '6px 0' }}><strong>Date Hired:</strong> {formatDate(detailsDriver.date_hired)}</p>
                  <p style={{ margin: '6px 0' }}><strong>Experience:</strong> {detailsDriver.experience_years != null && detailsDriver.experience_years !== '' ? `${detailsDriver.experience_years} year${Number(detailsDriver.experience_years) === 1 ? '' : 's'}` : '—'}</p>
                  <p style={{ margin: '6px 0' }}><strong>Hired By:</strong> {detailsDriver.hired_by || '—'}</p>
                  <p style={{ margin: '6px 0' }}><strong>Contract:</strong> {formatDate(detailsDriver.contract_start)} – {formatDate(detailsDriver.contract_end)}</p>
                </div>
              </div>
            </div>
          </div>
