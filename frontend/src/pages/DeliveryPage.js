 ...
 * current position).
 */
function ViewLocationMap({ pickupAddress, dropoffAddress, onEtaChange }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled || !pickup || !dropoff || !mapEl) {
        setStatus('failed');
        return;
      }
                  ...
                  {selectedDelivery.driver?.user ? (
                    <div className="map-driver-card">
                      ...
                      <div className="map-driver-info">
                        ...
                        <div className="map-driver-lastseen" style={{ color: '#888' }}>Live GPS not available yet — no driver app connected.</div>
                      </div>
                    </div>
                  ) : (
