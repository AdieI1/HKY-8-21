  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const staffRole = rolesData.find((r) => r.role_name?.toLowerCase() === 'staff')
        || rolesData.find((r) => r.role_name?.toLowerCase().includes('admin'));
      ...
      logsData.forEach((log) => {
        ...
        dynamicActivities.push({
          ...
          role: log.user?.role?.role_name || 'System',
          time: timeAgo(rawTime),
          timeMs: rawTime ? new Date(rawTime).getTime() : 0,
          rawDate: rawTime,
        });
            ...
            <div className="content-row">
              ...
              {logsView === 'recent' ? (
                <div className="content-section system-logs">
                  <div className="section-header">
                    <h3 className="section-title"><i className="fas fa-bolt" style={{ color: '#dc2626' }}></i> Activity Feed &amp; System Logs</h3>
                ...
                <div className="content-section all-system-logs">
                  <div className="section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      <h3 className="section-title" style={{ margin: 0 }}><i className="fas fa-history"></i> ALL ACTIVITY &amp; SYSTEM LOGS</h3>
                        ...
                        {['all', 'delivery', 'request', 'fleet', 'system'].map((cat) => (
                          <button
                            ...
                            style={{
                              ...
                              background: activityCategoryFilter === cat ? '#1e293b' : '#ffffff',
                              color: activityCategoryFilter === cat ? '#ffffff' : '#475569',
                              fontSize: 11,
                              fontWeight: 600,
                              cursor: 'pointer',
                  ...
                  <div className="section-content">
                    <table className="data-table logs-table">
                      ...
                      <tbody>
                        {activityFeed
                          .filter((item) => activityCategoryFilter === 'all' || item.category === activityCategoryFilter)
                          .map((item) => (
                            <tr key={item.id}>
                              <td style={{ textAlign: 'center' }}>
                                <i className={item.icon} style={{ color: item.color, fontSize: 15 }}></i>
                              </td>
                        ...
                        {activityFeed.filter((item) => activityCategoryFilter === 'all' || item.category === activityCategoryFilter).length === 0 && (
                          <tr><td colSpan="5" style={{ textAlign: 'center', padding: 24 }}>No activity records found in this category.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
